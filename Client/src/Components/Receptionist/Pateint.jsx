...
import {
  ...
  FiHeart, FiDroplet, FiUserPlus, FiInfo, FiEye, FiDownload
} from 'react-icons/fi';
...
export default function ReceptionistPatient() {
  ...
  const [currentQRPatient, setCurrentQRPatient] = useState(null);
  ...
  const handleDownloadQR = () => {
    ...
    if (svg) {
      ...
      img.onload = () => {
        ...
        downloadLink.download = `${currentQRPatient.name}-qr-code.png`;
            ...
            {filteredPatients.length > 0 ? filteredPatients.map((p) => (
              <motion.tr key={p._id} whileHover={{ scale: 1.01 }} className="transition">
                ...
                <td className="px-6 py-4 flex gap-3">
                  ...
                  <button onClick={() => handleShowQR(p)}><FiEye className="text-blue-500" /></button>
                </td>
              </motion.tr>
            )) : (
